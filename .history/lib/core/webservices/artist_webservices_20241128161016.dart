...
class ArtistWebservices {
  ...
  Future<ResponseData> getArtistList({required String keyword}) async {
    ...
    Future<ResponseData> getArtistAvailable({required String country, required String seller, required String page}) async {
    try {
      var token = _storageSession.readTokenJwt();
      var resp = await Api().get(url: AppEndpoint.getListArtistAvailable(country: country, seller: seller, page: page), headers: HttpHeader.headersWithAuth(token));
      return resp;
    } catch (e) {
      LoggerHelper.error("fetchArtist error --> $e");
      return ResponseError.defaultError();

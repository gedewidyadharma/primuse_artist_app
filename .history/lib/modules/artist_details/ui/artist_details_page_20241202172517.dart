    ...
    return Obx(
      () => SfCartesianChart(
        ...
        primaryXAxis: CategoryAxis(
          ...
          scrollbarSettings: const ScrollbarSettings(
            enable: true,
            height: 8,
          ),
          // Optional: Berapa banyak data yang terlihat
          visibleMinimum: controller.dataHistorySpotify.length > 6 
              ? controller.dataHistorySpotify.length - 6 
              : 0, // Menampilkan 6 data terakhir
          // Optional: Auto interval saat scroll
          labelRotation: 45, // Rotasi label agar tidak tumpang tindih
          majorGridLines: const MajorGridLines(width: 0), // Hapus grid lines
        ),
        
        // Y Axis configuration
        primaryYAxis: NumericAxis(
          numberFormat: NumberFormat.compact(), // Format angka ke compact
        ...
        series: <LineSeries<History, String>>[
          LineSeries<History, String>(
            dataSource: controller.dataHistorySpotify.value,
            xValueMapper: (History sales, _) => 
                DateFormat("MMM yy").format(sales.date!),
            yValueMapper: (History sales, _) => 
                sales.monthlyListenersCurrent,
            // Optional: Tambahkan marker
            markerSettings: const MarkerSettings(
              isVisible: true,
              height: 6,
              width: 6,
            ),
            // Optional: Tambahkan data label
            dataLabelSettings: const DataLabelSettings(
              isVisible: false, // Set true jika ingin menampilkan label
            ),
          ),
        ],
      ),

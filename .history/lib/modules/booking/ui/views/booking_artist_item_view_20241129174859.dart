              ...
              child: Column(
                ...
                children: [
                  _LabelValue(
                    ...
                    value: "${DateFormating.changeFormatFromString(format: "MMM dd, yyyy", date: artist.dates?.from ?? "")} - ${DateFormating.changeFormatFromString(format: "MMM dd, yyyy", date: artist.dates?.to ?? "")}",
                  ),
                  const SizedBox(height: 15),
                  _LabelValue(
                    label: "Regional Available",
                    value:  artist.territories?.join(', ').toUpperCase() ?? '',
                  ),
                ],
              ),
            )
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Text(value, style: Theme.of(context).textTheme.labelLarge!.copyWith(fontSize: 12)),
      ],
    );
  }
}
